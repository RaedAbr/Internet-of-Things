...
////////////////////////////////////////////////////////////
char DEVICE_EUI[]  = "00E3CA40B6ED2090";
char APP_EUI[] = "70B3D57ED00143BA";
char APP_KEY[] = "A96D51C43C02939506D55005C969CA74";
////////////////////////////////////////////////////////////
...
void setup() 
{
  ...
  if( ACC.check() == 0x32 ) 
  {
    USB.println(F("8. Switch Accelerometer ON OK"));     
  }
  else 
  {
    USB.print(F("8. Switch Accelerometer ON error = ")); 
    USB.println(error, DEC);
  }
}
...
void loop() 
{
  ...
  if( error == 0 ) 
  {
    ...
    // led
    /////////////////////////////
    int led1State = int(Utils.getLED(LED1));
    USB.print(F("led1 state: "));
    USB.println(led1State);
    if (led_state == 1) {
   ...
   //----------X Value-----------------------
    ...
    sprintf(data, "%04X%04X%04X%02X", x_acc, y_acc, z_acc, led_state);
    //  ========= ========= ========= ===========
    // | x_acc   | y_acc   | z_acc   | led1State |
    //  ========= ========= ========= ===========
    // | %04X    | %04x    | %04x    | %02X      |
    //  ========= ========= ========= ===========
    // | 16 bits | 16 bits | 16 bits | 8 bits    |
    //  ========= ========= ========= ===========
    USB.print(F("send data: "));
    USB.println(data);
    ...
    if( error == 0 ) 
    {
      USB.println(F("Data to be sent"));
      USB.print(x_acc); USB.print(" ");
      USB.print(y_acc); USB.print(" ");
      USB.print(z_acc); USB.println();
      ...
      { 
        ...
        if (strCmp(LoRaWAN._data, "31", 2) == 0) {
          led_state = 1;
        } else {
          led_state = 0;
        }
